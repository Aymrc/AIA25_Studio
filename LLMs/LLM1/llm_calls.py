...
def collect_design_parameters(user_input, messages):
    try:
        ...
        print(f"[DEBUG] Error in collect_design_parameters: {str(e)}")
        return {}
    ...
    if current_state == "initial":
        ...
        if is_uncertain:
            ...
            try:
                ...
                print(f"[DEBUG] Error parsing WWR: {str(e)}")
...
# Handler for user questions or change requests
def handle_change_or_question(user_input, design_data):
    ...
    if any(kw in lowered for kw in change_keywords):
        try:
            ...
            print(f"[DEBUG] Error processing change: {str(e)}")
    ...
    if any(kw in lowered for kw in improve_keywords):
        try:
            ...
            print(f"[DEBUG] Error suggesting improvements: {str(e)}")
            print(traceback.format_exc())
            return "complete", "I had trouble thinking of improvements. Could you ask more specifically what you'd like to improve?", design_data
    ...
    try:
        ...
        print(f"[DEBUG] Error in general Q&A: {str(e)}")
        print(traceback.format_exc())
