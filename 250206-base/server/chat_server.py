...
def chat_endpoint(req: ChatRequest):
    ...
    try:
        ...
        # UPDATE PHASE 1 COMPLETION DETECTION 06.06.25
        if new_state == "complete":
            response += "\n\n🎯 Geometry detected! Running ML analysis..."
           
            # Launch ML predictor when geometry is available
            try:
                predictor_path = os.path.join(os.path.dirname(__file__), "..", "utils", "ML_predictor.py")
                subprocess.Popen(["python", predictor_path])
                print("🚀 ML_predictor.py launched after geometry detection")
